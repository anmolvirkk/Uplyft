            ...
            <div className={styles.sideSection}>
                <div className={styles.slotSection}>
                    {slots[allRoutes['book']]&&slots[allRoutes['book']].length!==0 ? slots[allRoutes['book']].map((item)=>{
                        return <NavLink onClick={()=>setRoute(item.id)} key={item.id} to={`/journals/${allRoutes['book']}/${allRoutes['date']}/${item.id}`} className={styles.sideSectionSlot} activeClassName={styles.activeSectionSlot} data-title={item.title}><p>{item.title.replace(/\s/g, "") ==='' ? item.time : item.title}</p>
                ...
                {allRoutes['book']&&allRoutes[allRoutes['book']].slot?<Redirect from={`/journals/${allRoutes['book']}`} to={`/journals/${allRoutes['book']}/${allRoutes['date']}/${allRoutes[allRoutes['book']].slot}`} />:null}
            </div>
        )
    }else{
        return null
